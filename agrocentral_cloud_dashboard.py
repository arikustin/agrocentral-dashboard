"""
agrocentral_cloud_dashboard.py
================================
Dashboard AGROCENTRAL en Railway.
Lee datos de PostgreSQL subidos desde la PC local.

Variables de entorno en Railway:
    DATABASE_URL                 (automática)
    DASHBOARD_PASSWORD_ARIEL
    DASHBOARD_PASSWORD_USER2
    DASHBOARD_PASSWORD_USER3
"""
...
USERS = {
    ...
    "jere": os.environ.get("DASHBOARD_PASSWORD_USER2",  "agro2026"),
    "ale": os.environ.get("DASHBOARD_PASSWORD_USER3",  "agro2026b"),
}
...
def api_item(item_id):
    if not check_auth(request): return jsonify({}), 401
    try:
        return jsonify(db.get_item(item_id) or {})
...
def api_status():
    ...
    try:
        meta = db.get_sync_meta()
        return jsonify({
            "status":"done","progress":100,"cloud_mode":True,
            "message": f"Datos actualizados — {meta.get('total_items','?')} publicaciones",
...
def api_update():
    """En cloud mode no se puede actualizar desde el browser."""
    return jsonify({"ok":False,
        "msg":"Los datos se actualizan desde la PC de Ariel."})
...
<div class="hdr">
  ...
  <div style="text-align:right">
    ...
    <button class="btn-upd" id="bup" onclick="triggerUpdate()">🔄 Actualizar ahora</button>
...
<table id="mt">
  ...
  <tbody id="tb">
    <tr><td colspan="11"><div class="empty">
      <div class="ic">📊</div>
      <p>Presiona <strong>Actualizar ahora</strong> para cargar los datos.</p>
    </div></td></tr>
  </tbody>
</table>
</div>
